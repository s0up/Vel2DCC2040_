...
public class Vel2DCC2040_ implements PlugIn {
	...
	public void run(String arg) {
		...
		int width,height,xsteps,ysteps,yi,yj,dy,cont1=0,cont2=0,dir=5,dt=3000;
		double max=0,min=0,Startime=0,Endtime=0,diff,RMSmx=0,RMSmn=0;
		

		String strFilePath = "C://Users//Margaret//Desktop//Daniel//Write.txt";

		ImagePlus img = IJ.getImage();
		Calibration cal = img.getCalibration(); //Calibration objects contain an image's spatial and density calibration data
		ImageProcessor ip = img.getProcessor();
		ip.setCalibrationTable(cal.getCTable());
		ip.setInterpolate(true); // setInterpolate is a method of the class  ImageProcessor
        ...
        String yLabel = (cal !=null && cal.getValueUnit()!=null && !cal.getValueUnit().equals("Gray Value")) ?
            "Value ("+cal.getValueUnit()+")" : "Velocity ( mm/s)";
		...
		String buffer = "MaxVel\tMeanVel\tQ\tRMSmx\tRMSmn\n";
		String buffer2="a\tb\tc\td\te\tf\tg\th\ti\tj\tk\tl\tm\tn\to\tp\n";
		...
		for(int mySliceNumber = 1; mySliceNumber <= size-2; mySliceNumber++) {
			Startime=System.currentTimeMillis();//tpo de inicio
			...
			for(int k=0;k<xsteps;k++){//goes through the whole image in the radial direction
				for(int z=0;z<ysteps;z++){//looks for the template along the longitudinal direction					
	...
	//filter extreme values. i think this is valid but needs to be modified to work for every video
			...
			for(int j=0; j<xsteps;j++){//vertical average of velocities
				x[j]=j;
			...
			//Numeric integral of vel1 to calculate the volumetric flow rate (simpsons rule)
			double Q=0;
			
			for(int i=0;i<(int)Math.floor((vel1.length)/3);i++){
				Q+=(0.333)*(r[i+1]-r[i])*((vel1[i*3]*r[i*3])+(4*vel1[(i*3)+1]*r[(i*3)+1])+(vel1[(i*3)+2]*r[(i*3)+2]));				
			}
	        ...
	        IJ.showStatus("Blood flow is " + Q + " nl/s"); // prints the calculated flow rate on the imagej status bar
	        Endtime=System.currentTimeMillis();
	        diff=Endtime-Startime;
	        //IJ.showMessage("tiempo es" + diff);
	        buffer+=MaxVel;
	        buffer+= "\t";
		    buffer+=MeanVel;
		    buffer+= "\t";
...
//        systemClipboard.setContents(cont, cont);
    	...
    	StringSelection cont = new StringSelection(buffer2);
        systemClipboard.setContents(cont, cont);
        
        System.out.println("Clipboard set to : " + buffer2);
...
public static int [][] Filter1(int[][] x, int dir){
	...
	for(int j=0;j<b;j++){
		...
		for(int i=0;i<a;i++){
			if(x[i][j]!=0 && Math.abs(x[i][j])>2.1*min && dir==0){//note 1
				x[i][j]=(-1)*min;
			}else{
				if(x[i][j]!=0 && Math.abs(x[i][j])>2.1*min && dir==1){//note 2
					x[i][j]=min;
				}
			}
		}
...
/*
 * Note 1: if the value is not zero and the value
 * is more than 2 time the minimum value of the column and the flow is going down stream
 * the value at i,j is -minumim. 
 */
