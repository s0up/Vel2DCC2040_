...
public class Vel2DCC2040_ implements PlugIn {
	...
	public void run(String arg) {
		...
		Calibration cal = img.getCalibration(); // Calibration objects contain
												// an image's spatial and
		...
		for (int mySliceNumber = 1; mySliceNumber <= size - 2; mySliceNumber++) {
			...
			for (int k = 0; k < xsteps; k++) {// goes through the whole image in
			...
			// Numeric integral of vel1 to calculate the volumetric flow rate
			// (simpsons rule)
			double Q = 0;
		...
		try {
			FileOutputStream out = new FileOutputStream(new Config().getXmlPath());
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			System.out.println("File " + new Config().getXmlPath() + " not found!");
		}
		
		//TODO:  make it actually write the xml file from string buffer2
